...
test('關卡頁：一步一步走完第一課會累積 XP', async ({ page }) => {
  ...
  // 三層揭露：提示 → 預期結果 → 看答案，一次只開一層
  const reveal = page.getByTestId('reveal-1-1-0')
  await expect(reveal).toHaveAttribute('data-level', '0')
  await page.getByTestId('reveal-next-1-1-0').click()
  await expect(reveal).toHaveAttribute('data-level', '1')
  await expect(reveal).toContainText('提示')
  await page.getByTestId('reveal-next-1-1-0').click()
  await expect(reveal).toContainText('預期結果')
  await page.getByTestId('reveal-next-1-1-0').click()
  await expect(reveal).toContainText('參考答案')
  await expect(page.getByTestId('reveal-next-1-1-0')).toHaveCount(0)
  await page.screenshot({ path: `${shotDir}/step-exercise.png`, fullPage: true })
  ...
  await expect(page.getByTestId('step')).toHaveAttribute('data-kind', 'teach')
  // 講出來：3 個關鍵字都勾到才能過
  await expect(page.getByTestId('teach-1-1')).toBeDisabled()
  await page.getByTestId('teach-key-0').check()
  await page.getByTestId('teach-key-1').check()
  await page.getByTestId('teach-key-2').check()
  await expect(page.getByTestId('teach-1-1')).toBeEnabled()
...
test('地圖：每個世界有一支可跳過的暖身影片連結', async ({ page }) => {
  await page.goto('/map')
  for (const id of [1, 2, 3, 4, 5]) {
    const link = page.getByTestId(`world-video-${id}`)
    await expect(link).toContainText('暖身影片')
    await expect(link).toHaveAttribute('href', /youtube\.com\/watch\?v=/)
    await expect(link).toHaveAttribute('target', '_blank')
  }
})

test('卡住頁：貼錯誤訊息會命中字典；查不到才出現「問老師」', async ({ page }) => {
  await page.addInitScript(seed, SETUP_DONE)
  await page.goto('/day/1')
  await page.getByTestId('mini-stuck').click()
  await expect(page).toHaveURL(/\/stuck$/)
  await page.getByTestId('stuck-input').fill('ERROR:  column "Taipei" does not exist')
  await expect(page.getByTestId('stuck-hits')).toContainText('哪個字錯了')
  await expect(page.getByTestId('stuck-hits').getByTestId('stuck-entry-double-quote-value')).toBeVisible()
  await expect(page.getByTestId('stuck-miss')).toHaveCount(0)
  await page.screenshot({ path: `${shotDir}/stuck.png`, fullPage: true })
  await page.getByTestId('stuck-solved').click()
  await expect(page.getByTestId('toast')).toContainText('錯誤日誌')
  await page.getByTestId('stuck-input').fill('something totally unknown 12345')
  await expect(page.getByTestId('stuck-hits')).toHaveCount(0)
  await expect(page.getByTestId('stuck-ask')).toBeVisible()
})
