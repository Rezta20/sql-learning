...
- 學習方式（2026-09-10 起）：**單一電腦、個人使用**。練習題／專案題／Boss 都在 App 裡自己對答案（提示 → 預期結果 → 看答案）；卡住先查 App `/stuck` 錯誤字典；老師只在「開始」「字典查不到」「結算」出場。每個世界有一支可跳過的暖身影片。使用說明在 App `/guide` 與 `study/how-to.md`。
